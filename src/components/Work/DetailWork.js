...
export const DetailWork = () => {
  const {id} = useParams()
  const {name,tecnologies_used,preview_images,links} = getProjectById(id);
  const { github } = links;
  const website = links.website || false;
  return (
    <>
      ...
      <Tecnologies className="col d-flex flex-column align-items-left justify-content-center p-4">
        <TecnologiesTitle className="mb-2 p-0">Tecnologies used</TecnologiesTitle>
          ...
          <div className="row mt-3">
            ...
            {
              website && (<Button href={website} target="_blank" rel="noopener noreferrer">
